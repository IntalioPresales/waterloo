...
(function (e) { "function" === typeof define && define.amd ? define(["jquery"], e) : e(jQuery) })(function (e) {
    ...
    function g(a, b) {
        var c = function () { }, c = {
            autoSelectFirst: !1, appendTo: "body", serviceUrl: null, lookup: null, onSelect: null, width: a.clientWidth, minChars: 1, maxHeight: 300, deferRequestBy: 0, params: {}, formatResult: g.formatResult, delimiter: null, zIndex: 9999, type: "GET", noCache: !1, onSearchStart: c, onSearchComplete: c, containerClass: "autocomplete-suggestions", tabDisabled: !1, dataType: "text", lookupFilter: function (a, b, c) {
                //arabic diacritical marks
                var matchValues = stripTashkeel(a.value);
                var valueParts = matchValues.split(" ");
                var searchedVal = stripTashkeel(c.toLowerCase());
                var startWithSearch = $.grep(valueParts, function (e) {
                    return e.toLowerCase().startsWith(searchedVal);
                });
                var isValid = false;
                for (var i = -1; i < valueParts.length - 1 && !isValid; i++) {
                    var tmp = "";
                    for (j = i + 1; j < valueParts.length && !isValid; j++) {
                        tmp += valueParts[j] + " ";
                    }
                    if (tmp.toLowerCase().startsWith(searchedVal)) {
                        isValid = true;
                    }
                }
                if (startWithSearch.length > 0 || isValid) {
                    return true;
                }
                return false;
        ...
        this.initialize();
        this.setOptions(b)
    } var h = { extend: function (a, b) { return e.extend(a, b) }, createNode: function (a) { var b = document.createElement("div"); b.innerHTML = a; return b.firstChild } }; g.utils = h; e.Autocomplete = g; g.formatResult = function (a, b) { var c = "(" + b.replace(RegExp("(\\/|\\.|\\*|\\+|\\?|\\||\\(|\\)|\\[|\\]|\\{|\\}|\\\\)", "g"), "\\$1") + ")"; return a.value.replace(RegExp(c, ""), "<strong>$1</strong>") }; g.prototype = {
        ...
        setOptions: function (a) {
        ...
        }, clearCache: function () { this.cachedResponse = []; this.badQueries = [] }, clear: function () { this.clearCache(); this.currentValue = null; this.suggestions = [] }, disable: function () { this.disabled = !0 }, enable: function () { this.disabled = !1 }, fixPosition: function () {
            var a;
            "body" === this.options.appendTo && (a = this.el.offset(), e(this.suggestionsContainer).css({ top: a.top + this.el.outerHeight() + "px", left: a.left + "px" }))
            ...
            if (!b.disabled) {
                switch (a.keyCode) {
                    case 38: case 40: return
                }
                clearInterval(b.onChangeInterval);
                //if (b.currentValue !== b.el.val())
                if (0 < b.options.deferRequestBy)
                    b.onChangeInterval = setInterval(function () { b.onValueChange() }, b.options.deferRequestBy); else
                    b.onValueChange()
            }
            ...
            if (!b) return e.trim(a); a = a.split(b);
            ...
            if (b == ' ') {
                var array = [];
                for (var z = 0; z < this.options.lookup.length; z++) {
                    if (this.options.lookup[z].value.toString().contains(' ')) {
                        array.push(this.options.lookup[z])
                    }
                }
                return {
                    suggestions: array
        ...
        isBadQuery: function (a) {
            for (var b = this.badQueries, c = b.length; c--;)
                if (0 === a.indexOf(b[c])) return !0; return !1
        }, hide: function () {
        ...
        suggest: function () {
            if (0 === this.suggestions.length)
                ...
                var a = this.options.formatResult, b = this.getQuery(this.currentValue), c = this.classes.suggestion, d = this.classes.selected, f = e(this.suggestionsContainer), g = ""; e.each(this.suggestions, function (d, e) { g += '<div class="' + c + '" data-index="' + d + '">' + a(e, b) + "</div>" }); f.html(g).show(); this.visible = !0; this.options.autoSelectFirst && (this.selectedIndex = 0, f.children().first().addClass(d))
            }
        ...
        return 0 === arguments.length ? this.first().data("autocomplete") : this.each(function () {
            var c = e(this), d = c.data("autocomplete");
            if ("string" === typeof a) {
